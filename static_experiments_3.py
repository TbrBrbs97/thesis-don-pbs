...
for demand_scen in demand_scenarios:
    for demand_subscen in demand_subscenarios:
        lambdapeak = get_scenario_mean_demand('city', network_size, scen=demand_scen,
                                              subscen=demand_subscen, peak=1)
        mupeak = get_scenario_mean_demand('terminal', network_size, scen=demand_scen,
                                          subscen=demand_subscen, peak=1)

        if network_size != 'real':
            mean_demand = convert_md_todict(lambdapeak, mupeak, scen=demand_scen)
        else:
            mean_demand = convert_md_todict(lambdapeak, mupeak, scen=demand_scen, subscen=demand_subscen)

        sample = 0
        while sample < nb_of_samples:
            print('CURRENTLY RUNNING scenario: ', demand_scenario,  'and subscenario:', demand_subscen)
            random_seed = sample
            ...
            opt_name = 'Results/' + network_size + '_scen' + str(demand_scen) + '_subscen' + str(demand_subscen) + '_opt.pickle'
